...
class MidiCapsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {"midi_ids": midi_ids, "input_ids": txt["input_ids"].squeeze(0), "attention_mask": txt["attention_mask"].squeeze(0), "path": ex["path"]}
...
class PreTokenizedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {"midi_ids": m_ids, "input_ids": i_ids, "attention_mask": a_mask, "path": item.get("path", "unknown")}
...
def main():
    ...
    if os.path.exists(cfg.SAVE_FILE):
        print(f"Found checkpoint: {cfg.SAVE_FILE}. Resuming...")
        ...
        # Estimate start epoch based on total steps recorded in history
        steps_per_epoch = len(loader)
        if len(loss_history) > 0:
            start_epoch = len(loss_history) // steps_per_epoch
